...
export function OfflineAuthProvider({ children }: { children: ReactNode }) {
  ...
  const ensureDemoDataExists = async () => {
    try {
      ...
      if (!demoCompany) {
        logger.log('🏭 Creating demo company for offline functionality...')
        ...
        logger.log('✅ Demo company created')
      }
      ...
      if (!adminExists) {
        logger.log('👤 Creating demo admin user for offline functionality...')
        ...
        logger.log('✅ Demo admin user created')
      }
      ...
      if (!superAdminExists) {
        logger.log('👤 Creating demo super admin user for testing...')
        ...
        logger.log('✅ Demo super admin user created')
      }
  ...
  const createDemoProjectsAndLocations = async () => {
    try {
      ...
      if (demoProjects.length === 0) {
        logger.log('📁 Creating demo projects...')
        ...
        const demoProject = {
          id: 'demo-project-1',
          name: 'Edifício Demo - Escritórios',
          companyId: 'demo-company',
          floorPlanImages: [sampleFloorPlan],
          obraAddress: 'Rua das Flores, 123, Centro, São Paulo - SP',
          obraCEP: '01234-567',
          obraCNPJ: '12.345.678/0001-90',
          contratanteName: 'Construtora Demo Ltda',
          contratanteAddress: 'Av. Principal, 456, Vila Nova, São Paulo - SP',
          ...
          engenheiroResponsavelPadrao: 'Eng. João Silva',
          dispositivoDeAncoragemPadrao: 'Placa de Aço Inox',
        ...
        logger.log('✅ Demo project created')
      }
      ...
      if (demoLocations.length === 0) {
        logger.log('📍 Creating demo locations...')
        ...
        logger.log('✅ Demo locations created')
      }
      
    } catch (error) {
      logger.error('❌ Error creating demo projects/locations:', error)
    }
  }
      ...
      if (storedUserId && storedCompanyId) {
        ...
        if (user && company) {
          ...
          if (!currentCompany.trialEndDate && (!currentCompany.subscriptionPlan || currentCompany.subscriptionPlan === 'trial')) {
            logger.log('⚡ Fixing company without trialEndDate:', currentCompany.id)
            ...
            logger.log('✅ Company trial fixed:', {
          ...
          logger.log('✅ Restored offline session:', user.name)
    ...
    try {
      ...
      if (user) {
        ...
        if (company) {
          ...
          logger.log('✅ Offline login successful:', user.name)
        ...
        if (isOnline) {
          try {
            ...
            if (response.ok) {
              ...
              logger.log('✅ Online login successful, cached offline:', user.name)
            ...
            logger.error('Server login failed:', serverError)
            return { success: false, message: 'Erro de conexão com o servidor.' }
          }
        } else {
          return { success: false, message: 'Credenciais inválidas (offline).' }
        }
      }
    } catch (error) {
  ...
  const register = async (userData: RegisterData): Promise<{ success: boolean; message: string }> => {
    ...
    try {
      ...
      logger.log('✅ Offline registration successful:', user.name)
  ...
  const loginWithGoogle = async (): Promise<{ success: boolean; message: string }> => {
    ...
    try {
      ...
      // Em produção, aqui seria a integração real com Firebase Auth ou Google Auth
      logger.log('🔄 Simulando login com Google...')
      
      // Simular delay de autenticação
      await new Promise(resolve => setTimeout(resolve, 1500))
      
      // Criar usuário demo com dados do Google
      ...
      logger.log('✅ Google login successful (demo):', googleUser.name)
  ...
  const registerWithGoogle = async (): Promise<{ success: boolean; message: string }> => {
    // Para registro, usar a mesma lógica do login
