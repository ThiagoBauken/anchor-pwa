...
export function ReportsTab() {
  const { points, tests, setInspectionFlags, currentProject, users } = useAnchorData();
  ...
  const handleRunInspectionCheck = async () => {
    ...
    // Get all tests from storage, not just project-filtered ones for context
    ...
    try {
        ...
        if (result.pointsNeedingInspection) {
            setInspectionFlags(result.pointsNeedingInspection);
            if (result.pointsNeedingInspection.length > 0) {
              toast({ title: 'Verificação Concluída', description: `${result.pointsNeedingInspection.length} pontos precisam de inspeção e foram marcados.` });
            } else {
